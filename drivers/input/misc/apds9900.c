...
static int APDS9900_proximity_change_data(int pdata)
{
    ...
    for(i=proximity_data_tbl.last_data;i > 0;i--)
    {
      if(pdata > proximity_thres[i].hi){
        ret_data = i - 1;
        if(proximity_thres[i].ppcount != proximity_thres[ret_data].ppcount
        || proximity_thres[i].pdrive != proximity_thres[ret_data].pdrive){
          break;
        }
      }
      else {
...
{
    u8 wbuf[5] = {0};
    int last_level = proximity_data_tbl.last_data;
    ...
    if(proximity_thres[last_level].ppcount != proximity_thres[level].ppcount){
      reg_value_ppcount   = proximity_thres[level].ppcount;
    ...
    if(proximity_thres[last_level].pdrive != proximity_thres[level].pdrive){
      reg_value_control   = 0x20 | (proximity_thres[level].pdrive<<6);
    ...
    PROXIMITY_DPRINTK("DEBUG:%s lv:%d hi:%d lo:%d ppcount:%d pdrive:%d\n", __func__,
        level,reg_value_on_thres,reg_value_off_thres,reg_value_ppcount,reg_value_control);
}
...
static int APDS9900_hw_init(void)
{
    ...
    reg_value_control   = 0x20 | (proximity_thres[proximity_max_level].pdrive<<6);
    reg_value_ppcount   = proximity_thres[proximity_max_level].ppcount;
