...
#include "yas_mag_driver-yas532_v42602.c"
#include <mach/hs_io_ctl_a.h>
static int debug_log_enable = 0;
#define GS_DPRINTK(msg,...) \
    if(debug_log_enable) printk("[g_sensor]:dbglog:%d " msg,__LINE__,##__VA_ARGS__)
...
geomagnetic_enable(struct geomagnetic_data *data)
{
    if (!atomic_cmpxchg(&data->enable, 0, 1)) {
        GS_DPRINTK("DEBUG:geomagnetic sensor change enable\n");
...
geomagnetic_disable(struct geomagnetic_data *data)
{
    if (atomic_cmpxchg(&data->enable, 1, 0)) {
        GS_DPRINTK("DEBUG:geomagnetic sensor change disable\n");
        ...
        size_t count)
{
    ...
    GS_DPRINTK("START:geomagnetic_delay_store value is %d\n",value);
        ...
        size_t count)
{
    ...
    value = !!simple_strtol(buf, NULL, 10);
    GS_DPRINTK("START:geomagnetic_enable_store value is %d\n",value);
...
static ssize_t
geomagnetic_dbglog_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    debug_log_enable ^= 1;

    if(debug_log_enable){
        printk("[g_sensor] DEBUG: DEBUG_ON\n");
    }
    else{
        printk("[g_sensor] DEBUG: DEBUG_OFF\n");
    }
...
#endif /* DEBUG */
static DEVICE_ATTR(dbglog, S_IRUGO, geomagnetic_dbglog_show, NULL);
...
static ssize_t geomagnetic_testcoil_start
    (struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    printk("[geomagnetic] START:%s\n",__func__);

    i_Ret = __geomagnetic_testcoil_start(&sens_x, &sens_y);

    printk("[geomagnetic] END:%s\n",__func__);
...
static DEVICE_ATTR(testcoil, S_IRUGO, geomagnetic_testcoil_start, NULL);
...
static DEVICE_ATTR(static_matrix, S_IRUGO|S_IWUSR,
        geomagnetic_raw_static_matrix_show, geomagnetic_raw_static_matrix_store);
static DEVICE_ATTR(dynamic_matrix, S_IRUGO|S_IWUSR,
        geomagnetic_raw_dynamic_matrix_show, geomagnetic_raw_dynamic_matrix_store);
static DEVICE_ATTR(ellipsoid_mode, S_IRUGO|S_IWUSR,
        geomagnetic_raw_ellipsoid_mode_show, geomagnetic_raw_ellipsoid_mode_store);
static struct attribute *geomagnetic_raw_attributes[] = {
    &dev_attr_threshold.attr,
    &dev_attr_distortion.attr,
    &dev_attr_shape.attr,
    &dev_attr_offsets.attr,
    &dev_attr_testcoil.attr,
#ifdef YAS_MAG_MANUAL_OFFSET
...
geomagnetic_work(struct yas_mag_data *magdata)
{
    ...
    if (rt < 0) {
        printk("[g_sensor] ERR:geomagnetic_work measure failed[%d]\n", rt);
    }
    GS_DPRINTK("xy1y2 [%d][%d][%d] raw[%d][%d][%d]\n",
...
geomagnetic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if(HS_HW_ID_LAB <= hs_hw_id) {
        data->threshold = YAS_DEFAULT_MAGCALIB_THRESHOLD_LAB;
        for (i = 0; i < 3; i++) {
            data->distortion[i] = YAS_DEFAULT_MAGCALIB_DISTORTION_LAB;
        }
    }
    data->shape = 0;
    ...
    if (input_data == NULL) {
        rt = -ENOMEM;
        printk("[g_sensor] ERR:geomagnetic_probe Failed to allocate input_data device\n");
        goto err;
    }
    ...
    if (rt) {
        printk("[g_sensor] ERR:geomagnetic_probe Unable to register input_data device: %s\n",
               input_data->name);
        goto err;
    }
    data_registered = 1;
    ...
    if (rt) {
        printk("[g_sensor] ERR:geomagnetic_probe sysfs_create_group failed[%s]\n",
               input_data->name);
        goto err;
    }
    sysfs_created = 1;
    ...
    if (input_raw == NULL) {
        rt = -ENOMEM;
        printk("[g_sensor] ERR:geomagnetic_probe Failed to allocate input_raw device\n");
        goto err;
    }
    ...
    if (rt) {
        printk("[g_sensor] ERR:geomagnetic_probe Unable to register input_raw device: %s\n",
               input_raw->name);
        goto err;
    }
    raw_registered = 1;
    ...
    if ((rt = yas_mag_driver_init(&hwdep_driver)) < 0) {
        printk("[g_sensor] ERR:geomagnetic_probe yas_mag_driver_init failed[%d]\n", rt);
        goto err;
    }
    if (hwdep_driver.init != NULL) {
        if ((rt = hwdep_driver.init()) < 0) {
            printk("[g_sensor] ERR:geomagnetic_probe hwdep_driver.init() failed[%d]\n", rt);
            goto err;
        }
    }
    if (hwdep_driver.set_position != NULL) {
        if (hwdep_driver.set_position(CONFIG_INPUT_YAS_MAGNETOMETER_POSITION) < 0) {
            printk("[g_sensor] ERR:geomagnetic_probe hwdep_driver.set_position() failed[%d]\n", rt);
            goto err;
        }
    }
    if (hwdep_driver.get_offset != NULL) {
        if (hwdep_driver.get_offset(&data->driver_offset) < 0) {
            printk("[g_sensor] ERR:geomagnetic_probe hwdep_driver get_driver_state failed\n");
            goto err;
        }
    }
    ...
    if (hwdep_driver.get_filter != NULL) {
        if (hwdep_driver.get_filter(&filter) < 0) {
            printk("[g_sensor] ERR:geomagnetic_probe hwdep_driver get_filter failed\n");
            goto err;
        }
...
{
    gpio_tlmm_config(GPIO_CFG(GPIO_GS_INT, 0, GPIO_CFG_INPUT,GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
