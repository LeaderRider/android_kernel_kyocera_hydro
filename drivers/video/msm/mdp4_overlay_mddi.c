...
#include <linux/clk.h>
#include <mach/clk.h>
int oem_lcd_update_flg = 0;
int oem_lcd_update_check( void )
{
  return(oem_lcd_update_flg);
}
static struct clk *bus_clk = NULL;
void oem_lcd_busclk_fix( void )
{
  int clk_rate;
  if(bus_clk == NULL){
    bus_clk = clk_get(NULL, "ebi1_clk");
    if (IS_ERR(bus_clk)){
      bus_clk = NULL;
    }
  }
  if(bus_clk)
  {
    clk_rate = clk_get_rate(bus_clk);
    if(clk_rate < 300000000)
    {
      clk_set_rate(bus_clk, 0xFFFFFFFF);
    }
  }
  oem_lcd_update_flg = 1;
}
