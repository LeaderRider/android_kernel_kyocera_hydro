...
void oem_dev_lcdbl_first_blon( void )
{
  if(g_lcd_on_first_bl == TRUE)
  {
    ...
    /* bug fix[LCD Status Check] */
    /* 最初の表示更新完了前にDISP OFFが来ている場合が有るため。 */
...
static int dev_lcd_resister_init_sequence(void)
{
  ...
  LCD_CtrlWrite_Panel_S(0x3A, 0x77);
  command             = 0xB3;
  parameter[0]        =       0x02;
  parameter[1]        =       0x00;
  parameter[2]        =       0x00;
...
static int __init dev_lcd_init(void)
{
  ...
  if (!ret) {
    ...
    pinfo->bl_max   = 0xFF;
    pinfo->bl_min   = 0x01;
    pinfo->clk_rate =  368640000 / 2;
    pinfo->clk_min  =  368640000 / 2;
    pinfo->clk_max  =  368640000 / 2;
