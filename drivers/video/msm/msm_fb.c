			    ...
			    boolean op_enable)
{
	...
	case FB_BLANK_UNBLANK:
		if (!mfd->panel_power_on) {
			oem_dev_lcd_wakelock(0x08);
			oem_lcd_busclk_fix();
			msleep(16);
			ret = pdata->on(mfd->pdev);
			oem_lcd_update_flg = 0;
			oem_dev_lcd_wakeunlock(0x08);
			if (ret == 0) {
			      ...
			      struct fb_info *info)
{
	...
	if(lcd_is_shutdown == TRUE){
		printk("LCD already shutdown!!! \n");
		return -EPERM;
	}
/*******************************/
	/*
			...
			unsigned long arg)
{
	...
	case MSMFB_MDP_PP:
        ...
        case MSMFB_FREX_CHECK:
{
         boolean frex_check;
         if(oem_dev_lcd_status_get() == FALSE)
         {
           frex_check = FALSE;
         }
         else
         {
           frex_check = oem_dev_lcd_testmode_frex_check();
         }
         ret = copy_to_user(argp, &frex_check, sizeof(frex_check));
}
          break;
        case MSMFB_LCD_STATUS_GET:
{
         boolean status;
         status = oem_dev_lcd_status_get();
         ret = copy_to_user(argp, &status, sizeof(status));
}
          break;
        case MSMFB_LCD_BACKLIGHT:
{
	        unsigned int data;
            unsigned int value;
            ret = copy_from_user(&data, argp, sizeof(data));
            ...
            value  = data & 0x3FFFFFFF;
            oem_dev_lcdbl_set(value);
            oem_dev_lcd_backlight_enable();
}
          break;
        case MSMFB_LCD_BACKLIGHT_OFF:
          oem_dev_lcdbl_set(0);
