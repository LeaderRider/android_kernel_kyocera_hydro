...
static bool msm_battery_was_initialized = false;
...
static void msm_batt_update_psy_status(void)
{
	...
	batt_capacity = rep_batt_chg.v1.batt_capacity;
...
*/
	DBG_LIMIT("BATT: rcvd: %d, %d, %d, %d; %d, %d, %d\n",
		 charger_status, charger_type, battery_status,
		 battery_level, battery_voltage, battery_temp, batt_capacity);
...
static void msm_batt_oem_update_status_cb(struct work_struct *work)
{
	printk("%s() called\n", __func__);
	msm_batt_update_psy_status();
	schedule_delayed_work(&msm_batt_info.dwork, msecs_to_jiffies(5*1000));
}

void update_msm_batt_status(void)
{
  msm_batt_update_psy_status();
}
EXPORT_SYMBOL(update_msm_batt_status);

void set_msm_batt_status_usb_otg(void)
{
  if( msm_battery_was_initialized )
  {
    wake_lock_timeout(&msm_battery_wake_lock, 1 * HZ);
    cancel_delayed_work_sync(&msm_batt_info.dwork);
    schedule_delayed_work(&msm_batt_info.dwork, msecs_to_jiffies(500));
  }
}
EXPORT_SYMBOL(set_msm_batt_status_usb_otg);
...
static int __devinit msm_batt_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&msm_batt_info.dwork, msm_batt_oem_update_status_cb);
	schedule_delayed_work(&msm_batt_info.dwork, msecs_to_jiffies(5*1000));
	wake_lock_init(&msm_battery_wake_lock, WAKE_LOCK_SUSPEND, "msm_battery");
	msm_battery_was_initialized = true;
	return 0;
}
