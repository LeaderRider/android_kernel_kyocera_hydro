...
番号                 日付      コメント
------------------   --------  ----------------------------------------------
G01-SWD0228-00046    12/04/18  品管情報保存処理
G01-SWD0111-00056    12/06/01  電池電圧監視のUSB接続検出改善
===========================================================================*/
...
/* G01-SWD0111-00056 ADD-S */
extern void set_msm_batt_status_usb_otg(void);
/* G01-SWD0111-00056 ADD-E */
...
/* G01-SWD0228-00046 USB抜去のタイミングで品管情報をNVに保存 ADD-S */
extern void touchpanel_touchcnt_write_nv_ex(void);
extern unsigned int* get_keypress_cnt_result(int* array_len);
extern int pmic_keypad_pressed_num_set_start(uint *pressed_num, uint len);
extern void clear_keypress_cnt(void);
static void msm_otg_save_device_det_info(void)
{
	/* タッチパネル */
	touchpanel_touchcnt_write_nv_ex();
	/* HWキー */
	{
		unsigned int *keydata;
		int keylen;

		keydata = get_keypress_cnt_result(&keylen);
		pmic_keypad_pressed_num_set_start(keydata, keylen);
		clear_keypress_cnt();
	}
}
/* G01-SWD0228-00046 USB抜去のタイミングで品管情報をNVに保存 ADD-E */
