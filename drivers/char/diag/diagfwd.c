...
#ifndef FEATURE_KYOCERA_DATA_QCOM
//#include "linux/oem_msg.h" 
extern int oemmsg_process_pkt(int cmd, int sub_id, int sub_cmd);
#define DIAG_OEM_LINUX 0xfc 
#endif /* FEATURE_KYOCERA_DATA_QCOM */
...
#ifndef FEATURE_KYOCERA_DATA_QCOM
static uint8_t disconnect_event_config[] = 
{
	0x60, 0x00, 0x12, 0x6a, 0x7e
};
static uint8_t disconnect_log_config[] = 
{
	0x73, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xda, 0x81, 0x7e
};
static uint8_t disconnect_msg_config[] = 
{ 
...
static int diag_process_apps_pkt(unsigned char *buf, int len)
{
    ...
    if(oemmsg_process_pkt(cmd_code, subsys_id, subsys_cmd_code))
    {
      return packet_type;
    }
    temp = buf;
...
int diagfwd_disconnect(void)
{
	...
	if (driver->logging_mode != MEMORY_DEVICE_MODE)
	{
		...
		diag_process_hdlc(disconnect_event_config, sizeof(disconnect_event_config));
		diag_process_hdlc(disconnect_log_config, sizeof(disconnect_log_config));
		diag_process_hdlc(disconnect_msg_config, sizeof(disconnect_msg_config));
	}
...
#else  /* FEATURE_KYOCERA_DATA_QCOM */
    if ((driver->logging_mode == USB_MODE)||(*(uint8_t *)driver->usb_buf_out == DIAG_OEM_LINUX)||
        ((*(uint8_t *)driver->usb_buf_out == CONTROL_CHAR)&&(*((uint8_t *)driver->usb_buf_out + 1)== DIAG_OEM_LINUX)))
    {
		diag_process_hdlc(driver->usb_buf_out, driver->read_len_legacy);
	}
#endif /* FEATURE_KYOCERA_DATA_QCOM */
