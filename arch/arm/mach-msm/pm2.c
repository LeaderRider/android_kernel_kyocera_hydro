...
#define ADDR_PANIC_INFO	(CONFIG_ANDROID_RAM_CONSOLE_EARLY_ADDR + CONFIG_ANDROID_RAM_CONSOLE_EARLY_SIZE)
static int kcj_write_android_crash_info( const char *cmd )
{
        int                     reset_kind = 0;
        unsigned char           *pPanicInfo = (unsigned char *)ADDR_PANIC_INFO;

        /*--------------------------------------------------------------------------*/

        memset( pPanicInfo, 0x00, 16 );         /* Clear magic code buffer */

        if ( (NULL != cmd) && (!strcmp(cmd, "android_system_crash") || !strcmp(cmd, "kernel_panic")) )
        {
            ...
            restart_reason = 0x776655AA;        /* replace restart_reason */
        }
