...
extern int oem_lcd_update_check( void );
				 ...
				 enum setrate_reason reason)
{
	...
	if (tgt_s->axi_clk_hz > strt_s->axi_clk_hz) {
		...
		if(oem_lcd_update_check()){
			rc = clk_set_rate(drv_state.ebi1_clk, 0xFFFFFFFF/* MAX */);
		}else{
	...
	if (tgt_s->axi_clk_hz < strt_s->axi_clk_hz) {
		if(oem_lcd_update_check()){
			res = clk_set_rate(drv_state.ebi1_clk, 0xFFFFFFFF/* MAX */);
		}else{
	...
	if(clk_info == NULL){
		clk_info = smem_get_entry(SMEM_SCORPION_FREQUENCY, &ssf_size);
	}
	if(clk_info != NULL){
		*clk_info =  tgt_s->acpu_clk_khz;
	}
