...
#define ONCRPC_PM_OEM_LED_CONTROL 200

#define KEYPAD_PRESSED_NUM_PROC 300
...
int pmic_oem_led_control(struct pm_oem_led_control_data *param)
{
	return pmic_rpc_set_struct(0, 0, (uint *)param, sizeof(*param),
			ONCRPC_PM_OEM_LED_CONTROL);
}
EXPORT_SYMBOL(pmic_oem_led_control);

/*
 * KeyPad
 */
int pmic_keypad_pressed_num_set_start(uint *pressed_num, uint len)
{
	return pmic_rpc_set_struct(0, 0, pressed_num, len,
				KEYPAD_PRESSED_NUM_PROC);
}
EXPORT_SYMBOL(pmic_keypad_pressed_num_set_start);
